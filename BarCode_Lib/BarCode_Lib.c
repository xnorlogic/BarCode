...
bool barCode_intg_check(unsigned char *data){
	...
	*******************************************/
	...
	for(Index = 0U ; Index < ArrayDataSize ; Index++){
		...
		/* Assemble the array element */
		MY_APP_BarCode.Data[Index] = (unsigned short int)((HighByte << 8 ) | LowByte);
		/* two byte offset */
		OffSet 		= OffSet + ByteOffset;
	}
...
bool barCode_loadNready(unsigned char *data){
	...
	*******************************************/
	...
	for(Index = 0U ; Index < ArrayDataSize ; Index++){
		...
		/* Assemble the array element */
		MY_APP_BarCode.Data[Index] = (unsigned short int)((HighByte << 8U ) | LowByte);
		/* two byte offset */
		OffSet 		= OffSet + ByteOffset;
	}
...
unsigned char CheckSum_Calculator(unsigned char BarCodeData_Size, unsigned char* data){
	...
	for (Index = 0 ; Index < BarCodeData_Size - 1U ; Index++){
		...
		CheckSUM_calc = (unsigned char)(CheckSUM_calc + ((255U - BYTE_ELEMENT) + 1U));
	}
